...
.factory('CustomerService', ['$http', '$q', 'UtilConfig', 'UtilTools',
        function($http, $q, UtilConfig, UtilTools) {
            var base = UtilConfig.httpurl;
            var config = UtilConfig.headconfig;
            //默认页长
            var PAGE_LENGTH = 20;
            var customerService = {
                 postFile: function(obj){
                        var delay = $q.defer();
                        var str = UtilTools.toKeyValue(obj);

                        $http.post(base + 'upload.php', str, config).success(function(msg) {
                            delay.resolve(msg);
                        }).error(function() {
                            delay.reject("can't not get weiboinfos!");
                        })
...
.factory('UtilTools', ['$rootScope', '$location', '$window',
        function($rootScope, $location, $window) {
            ...
            var tool = {
                ...
                windowOpen: function(url, target) {
                    ...
                    $window.open(url,target,'height=500,width=600,scrollbars=yes,location=no,top=300,left=500');
                },
                // 从数组中过滤出满足条件的项，并返回一个新数组
                ...
                // 从数组中查找出满足条件的第一个项，并返回它的索引
                find: function(obj, iterator, context){
                    var self = this;
                    var result;
                    self.any(obj, function(value, index, list) {
                        if (iterator.call(context, value, index, list)) {
                            result = value;
                            return true;
                        }
...
factory('service1', ['$q', '$http', 'UtilConfig', 'UtilTools', function($q, $http, UtilConfig, UtilTools) {
	...
	return {
		...
		getData: function(){
            ...
            $http.post(base + '1.json', str, config).success(function(msg) {
                delay.resolve(msg);
            }).error(function() {
                delay.reject("can't not get weiboinfos!");
            })
